<script>
   import Crash from "$lib/provably-fair/crash.svelte";
    ...
    $: tab = $seaser[0] || "classic Dice"

   const handleSellectGame = ((event)=>{
    goto(`${$url === "/" ? "" : $url}/?tab=${event?.name}`)
    ...
    const handleClientSeed = (async(event)=>{
        goto(`${$url === "/" ? "" : $url}/?tab=${tab}&clientseed=${event.client_seed}&Serverseed=${event.server_seed}&nonce=${event.nonce}`)
        const { result } = await handleFetch({server_seed:event.server_seed,
            client_seed: event.client_seed, 
            nonce: event.nonce
        })
        pint = (result?.result)
    })
...
<div class="sc-fSDTwv fSBjam">
    ...
    <div class="sc-jJoQJp gOHquD select {is_open ? "is-open" : ""} ">
        <button on:click={()=> is_open =! is_open} class="select-trigger">
            {tab || ""}
        <div class="arrow cMPLfC">
            <svg class="sc-gsDKAQ hxODWG icon " fill="none" viewBox="0 0 24 24" height="15" width="15" xmlns="http://www.w3.org/2000/svg">
